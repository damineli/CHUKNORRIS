# Run kymograph analysis scripts for a single kymograph
################################################################################
...
fl.nm <- "Arab_Col-0_Kymo_190815-A.txt"  # *** MUST SPECIFY! ***
## Specify channel name in the kymograph
var.nm <- "YC"  # can basically be anything between ""
#-------------------------------------------------------------------------------
...
image.par <- list(
  ...
  time.cut = 5 # integer < 5 to avoid problems with color scale
)
...
# in "KymoTimeSeries" and the tip aligned kymograph in "TipAlignedKymo"
...
# "KymoTimeSeries" contain columns sequentially named: 
...
# "TipAlignedKymo" is a matrix with the filtered fluorescence including
# smoothing and backgroud subtraction.
# Time increases through rows (from top to bottom) and Object length increases
...
#-------------------------------------------------------------------------------
# Read kymograph 
kymo <- as.matrix(read.table(paste(in.path, fl.nm, sep = ""), header = FALSE))

kymo.lst <- AnalyzeKymograph(kymo, tip.find.par, image.par, ts.par,filter.par, 
                             wvlt.par, out.par, var.nm, fl.nm)
#-------------------------------------------------------------------------------